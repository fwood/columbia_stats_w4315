...
\frame[t]{
  ...
  \[\bm{s^2}\{\bm{b}\}=-\Big(\frac{\partial^2 log L(\beta)}{\partial \beta^2}\Big)^{-1}\]
  \vskip 12pt
  \item
  ...
  \vskip 12pt
  \item
  These estimations are routinely provided by standard softwares.
  \end{itemize}
}
...
\frame[t]{
  ...
  \begin{itemize}
  \item
  Testing and confidence interval building for a single parameter.
  \item
  The covariance matrix of the estimators is given by the Hessian matrix of loglikelihood function.
  \[\bm{s^2}\{\bm{b}\}=-\Big(\frac{\partial^2 log L(\beta)}{\partial \beta^2}\Big)^{-1}\]
  \vskip 12pt
