...
\end{eqnarray*}
\item The sampling distribution for $b_1$ is the distribution of $b_1$ that arises from the variability of $b_1$ when the predictor variables $X_i$ are held fixed and the errors are repeatedly sampled
...
\frame[t] {
 \frametitle{Normality of $b_1's$ Sampling Distribution}
Since $b_1$ is a linear combination of the $Y_i's$ and each $Y_i = \beta_1 X_i + \beta_0 + e_i$ is (conditioned on $X_i, \beta_1,$ and $\beta_0$)
an independent normal random variable, then then distribution of $b_1$ under sampling of the errors is
normal as well
\begin{eqnarray*}
...
This means $b_1 \sim N(\Ave{b_1}, \Var{b_1} )$.\newline

To use this we must know $\Ave{b_1}$ and $\Var{b_1}$.
}
...
So now we know the mean of the sampling distribution of $b_1$ and conveniently (importantly) it's centered on the {\em true} value of the unknown quantity $\beta_1$ (the slope of the linear relationship).
 }
...
How does this behave as a function of $\sigma^2$ and the spread of the $X_i$'s?  Is this intuitive? 
Note: this assumes that we know $\sigma^2$. Can we?
...
\begin{itemize}
\item When we don't know $\sigma^2$ then one thing that we can do is to replace it with the MSE estimate of the same
\item Let
...
\begin{eqnarray*}
\Var{b_1} &=&  \frac{\sigma^2}{\sum(X_i - \bar X)^2} \\
\EstVar{b_1} &=&  \frac{s^2}{\sum(X_i - \bar X)^2}
\end{eqnarray*}
\end{itemize}
}
...
\begin{equation}b_1 \sim \mathcal{N}(\beta_1,  \frac{\sigma^2}{\sum(X_i - \bar X)^2})\label{b1_dist}  \end{equation} 
\item When $\sigma^2$ is unknown we have an unbiased point estimator of $\sigma^2$ 
\[\EstVar{b_1} =  \frac{s^2}{\sum(X_i - \bar X)^2}\]
\item As $n \rightarrow \infty$ (i.e. the number of observations grows large) $\EstVar{b_1} \rightarrow \Var{b_1}$ and we can use Eqn.~\ref{b1_dist}.
\item Questions
\begin{itemize}
...
\begin{itemize}
\item $b_1$ is normally distributed so $(b_1-\beta_1)/(\sqrt{\Var{b_1}})$ is a standard normal variable.  Why?
\item We don't know $\Var{b_1}$ because we don't know $\sigma^2$ so it must be estimated from data.  We have already denoted it's estimate $\EstVar{b_1}$
\item If using the estimate $\EstVar{b_1}$ we will show that
$$\frac{b_1-\beta_1}{\EstStd{b_1}} \sim t(n-2)$$
where
$$\EstStd{b_1} = \sqrt{ \EstVar{b_1}}$$
\end{itemize}
}
...
\bigskip
\begin{alertblock}{Cochran's Theorem}
...
normal and $\chi^2$ respectively).  The following random variable is defined to be a t-distributed random
variable:
...
following rewrite
$$\frac{b_1 - \beta_1}{{\EstStd{b_1}}} = \frac{\frac{b_1 - \beta_1}{{\Std{b_1}}}}{\frac{\EstStd{b_1}}{ \Std{b_1}}}$$
where 

$$\frac{\EstStd{b_1}}{ \Std{b_1}}= \sqrt{\frac{\EstVar{b_1}}{ \Var{b_1}}}$$}

\frame[t] {
 \frametitle{Studentized statistic cont.}
And note the following
$$\frac{\EstVar{b_1}}{ \Var{b_1}}= \frac{\frac{MSE}{\sum(X_i-\bar X)^2}}{\frac{\sigma^2}{\sum(X_i-\bar X)^2}} = \frac{MSE}{\sigma^2} = \frac{SSE}{\sigma^2(n-2)}$$
where we know (by the given simple version of Cochran's theorem) that the distribution of the last
